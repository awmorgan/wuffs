...
func GenerateWithResolver(args []string, resolveUse func(string) ([]byte, error)) ([]byte, error) {
	...
	return generate.GenerateWithResolver(&flags, args, func(pkgName string, tm *t.Map, files []*a.File) ([]byte, error) {
		...
		if *standaloneFlag || pkgName == "main" {
			...
			buf.WriteString("#include \"gen/c/wuffs-base.c\"\n")
			buf.WriteString("#include \"std/os/wuffs_os.h\"\n\n")
			buf.Write(unformatted)
			buf.WriteString("\n\n#ifdef __cplusplus\nextern \"C\" {\n#endif\n\n" +
...
func (g *gen) genIncludes(b *buffer) error {
	...
	b.writes("#include \"gen/c/wuffs-base.c\"\n")
	for _, use := range usesList {
		b.printf("#include \"gen/c/wuffs-%s.c\"\n",
